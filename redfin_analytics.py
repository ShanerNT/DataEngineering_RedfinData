...
target_bucket_name = config["aws"]["S3_BUCKET_NAME"]
target_bucket_folder = 'redfin'
target_bucket_raw = 'raw'
...
def transform_data(task_instance):
    ...
    s3_client.put_object(Bucket=target_bucket_name, Key=f"{target_bucket_folder}/{object_key}", Body=csv_data)
...
with DAG('redfin_analytics_dag',
        ...
        load_to_s3 = BashOperator(
            ...
            bash_command = 'aws s3 mv {{ ti.xcom_pull("tsk_extract_redfin_data")[0]}} s3://' +target_bucket_name+'/'+target_bucket_folder+'/'+target_bucket_raw+'/',
        )
